...
export interface ClientOptions {
  /**
   * Defaults to process.env['DIMONA_USA_API_API_KEY'].
   */
   ...
   *
   * Defaults to process.env['DIMONA_USA_API_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['DIMONA_USA_API_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Dimona Usa API API.
 */
export class DimonaUsaAPI {
  apiKey: string;
   ...
   * API Client for interfacing with the Dimona Usa API API.
   *
   * @param {string | undefined} [opts.apiKey=process.env['DIMONA_USA_API_API_KEY'] ?? undefined]
   * @param {string} [opts.baseURL=process.env['DIMONA_USA_API_BASE_URL'] ?? https://admin.dimonatee.com] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('DIMONA_USA_API_BASE_URL'),
    apiKey = readEnv('DIMONA_USA_API_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.DimonaUsaAPIError(
        "The DIMONA_USA_API_API_KEY environment variable is missing or empty; either provide it, or instantiate the DimonaUsaAPI client with an apiKey option, like new DimonaUsaAPI({ apiKey: 'My API Key' }).",
      );
    }
    ...
    const options: ClientOptions = {
      apiKey,
      ...opts,
      baseURL: baseURL || `https://admin.dimonatee.com`,
    };

    this.baseURL = options.baseURL!;
    this.timeout = options.timeout ?? DimonaUsaAPI.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('DIMONA_USA_API_LOG'), "process.env['DIMONA_USA_API_LOG']", this) ??
      defaultLogLevel;
  ...
  static DimonaUsaAPI = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static DimonaUsaAPIError = Errors.DimonaUsaAPIError;
