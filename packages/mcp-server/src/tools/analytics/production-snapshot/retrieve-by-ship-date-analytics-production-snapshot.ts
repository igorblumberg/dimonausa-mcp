...
export const handler = async (client: DimonaUsaAPI, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(
      await maybeFilter(jq_filter, await client.analytics.productionSnapshot.retrieveByShipDate(body)),
    );
  } catch (error) {
    if (isJqError(error)) {
